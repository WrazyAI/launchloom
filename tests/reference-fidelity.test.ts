...
describe("reference fidelity validator", () => {
  ...
  it("reports visual patterns but blocks only the token collision", () => {
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: `${validExperience} <div data-layout="generic-split-hero" />`,
      stylesSource: `:root { --ink: #fff; }`,
      motionSource: validMotion,
    });
    expect(report.pass).toBe(false);
    expect(report.visualPass).toBe(false);
    expect(report.visualFindings).toContainEqual(
      expect.objectContaining({ code: "prohibited-pattern" }),
    );
  ...
  it("ignores prohibited words outside relevant attribute values", () => {
    const report = validateReferenceCandidate({
      referenceDna: {
        ...dna,
        prohibitedPatterns: [...dna.prohibitedPatterns, "cards", "grid"],
      },
  ...
  it.each([
    ...
    "className",
    "class",
  ])("detects prohibited patterns in %s values", (attribute) => {
    const report = validateReferenceCandidate({
      referenceDna: {
        ...dna,
        prohibitedPatterns: [...dna.prohibitedPatterns, "cards"],
      },
  ...
  it("accepts human-readable marker values that normalize to the contract slugs", () => {
    const humanReadable = validExperience
      .replace(
        "single-column-editorial-chapters",
        "single column editorial chapters",
      )
    ...
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: humanReadable,
      stylesSource: validStyles,
      motionSource: validMotion,
    });
    expect(report.pass).toBe(true);
  ...
  it("rejects duplicated sections from the assigned rhythm", () => {
    const duplicated = validExperience.replace(
      '<section data-reference-section="image-chapter"></section>',
      '<section data-reference-section="image-chapter"></section><section data-reference-section="image-chapter"></section>',
    );
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: duplicated,
      stylesSource: validStyles,
      motionSource: validMotion,
    });
  ...
  it("does not count reference section names mentioned only in comments", () => {
    const unmarked = validExperience
      .replaceAll(/\sdata-reference-section="[^"]+"/gu, "")
      .replace(
        "<main ",
        "{/* hero image-chapter editorial-intro image-mosaic magazine-archive closing-scene */}<main ",
      );
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: unmarked,
      stylesSource: validStyles,
      motionSource: validMotion,
    });
...
${validExperience
  .replace(
    'data-mobile-recomposition="single-column-editorial-chapters"',
    "data-mobile-recomposition={mobile}",
  )
  .replace(
    'data-motion-primitive="masked-image-reveal"',
    "data-motion-primitive={motion}",
  )
  .replace(
    'data-navigation-geometry="quiet-corner-links"',
    "data-navigation-geometry={navigation}",
  )
  .replace(
    'data-hero-geometry="typographic-monument"',
    "data-hero-geometry={geometry}",
  )
  .replace(
    'data-service-presentation="magazine-archive-ledger"',
    "data-service-presentation={services}",
  )
  .replace(
    'data-cta-placement="after-hero-image"',
    "data-cta-placement={cta}",
  )}`;
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: staticMarkers,
      stylesSource: validStyles,
      motionSource: validMotion,
    });
    expect(report.findings).toEqual([]);
  ...
  it("resolves a statically bound section marker in the authored order", () => {
    const withStaticSectionId = `const opening = "hero";\n${validExperience.replace(
      'data-reference-section="hero"',
      "data-reference-section={opening}",
    )}`;
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: withStaticSectionId,
      stylesSource: validStyles,
      motionSource: validMotion,
    });
...
export default function Experience({ content }) { return <main data-mobile-recomposition="single-column-editorial-chapters" data-motion-primitive="masked-image-reveal"><nav data-navigation-geometry="quiet-corner-links" /><Hero content={content} /><Services content={content} /><Faqs content={content} /><section id="contact"><a data-early-conversion data-cta-placement="after-hero-image" /></section></main>; }`;
  ...
  it("handles zero-prop authored components while tracing sealed content", () => {
    const source = `function Accent() { return <span aria-hidden="true" />; }\n${validExperience.replace(
      '<nav data-navigation-geometry="quiet-corner-links">',
      '<Accent /><nav data-navigation-geometry="quiet-corner-links">',
    )}`;
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: source,
  ...
  const tokenFindings = (experienceSource: string) =>
  ...
  it.each([
    [
      ...
      `const { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } = content; return ${aliasedExperience};`,
    ],
    [
      "chained aliases",
      `const sealed = content; const { hero: banner, services: serviceItems, faqs: faqItems } = sealed; const heroImage = banner.image; return ${aliasedExperience};`,
    ],
    [
      "destructured props",
      `export default function Experience({ content: { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } }) { return ${aliasedExperience}; }`,
    ],
    [
      "renamed content parameter",
      `export default ({ content: sealed }) => { const { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } = sealed; return ${aliasedExperience}; };`,
    ],
    [
      "ordinary props parameter",
      `export default function Experience(props) { const { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } = props.content; return ${aliasedExperience}; }`,
    ],
    [
      "returned JSX alias",
      `export function Experience({ content }) { const view = ${validExperience}; return view; }`,
    ],
    [
      "separate default export",
      `const Page = ({ content }) => ${validExperience}; export default Page;`,
    ],
    [
      "direct content parameter",
      `export function Experience(content) { return ${validExperience}; }`,
    ],
    [
      "block output",
      `if (ready) { const { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } = content; return ${aliasedExperience}; }`,
    ],
    [
      ...
      validExperience
        .replace(
          "{content.services}",
          "{content.services.map(({ name }) => <p>{name}</p>)}",
        )
        .replace(
          "{content.faqs}",
          "{content.faqs.map((faq) => { const answer = <p>{faq.answer}</p>; return answer; })}",
        ),
    ],
    [
      "mapped aliases",
      `export const Experience = ({ content }) => { const { hero: { image: heroImage }, services, faqs } = content; const serviceItems = services.map(({ name }) => <p>{name}</p>); const faqItems = faqs.map(({ question }) => <p>{question}</p>); return ${aliasedExperience}; };`,
    ],
    [
      "returned output expressions",
      "export function Experience({ content }) { const { hero: { image }, services, faqs } = content; return [image, services.map(({ name }) => name), faqs.map(({ answer }) => answer)]; }",
    ],
    [
      "typed and bracketed aliases",
      `export function Experience({ content }) { const heroImage = (content["hero"]["image"] as string); const serviceItems = content.services!; const faqItems = content?.faqs; return ${aliasedExperience}; }`,
    ],
    [
      "unrelated block shadow",
      `const { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } = content; { const heroImage = "local"; const serviceItems = []; const faqItems = []; } return ${aliasedExperience};`,
    ],
    [
      "mapping callback output",
      "export function Experience({ content }) { return [1].map(() => <div><img src={content.hero.image} />{content.services}{content.faqs}</div>); }",
    ],
  ...
  it.each([
    [
      ...
      `const { hero: { image: heroImage }, services, faqs } = content; return ${staticExperience};`,
    ],
    [
      "unused assignments",
      `const image = content.hero.image; const services = content.services; const faqs = content.faqs; return ${staticExperience};`,
    ],
    [
      "unused parameter bindings",
      `export default function Experience({ content: { hero: { image }, services, faqs } }) { return ${staticExperience}; }`,
    ],
    [
      "unused JSX assignment",
      `export function Experience({ content }) { const unused = ${validExperience}; return ${staticExperience}; }`,
    ],
    [
      "unused mapped output",
      `const images = [content.hero.image].map((image) => <img src={image} />); const services = content.services.map((service) => <p>{service.name}</p>); const faqs = content.faqs.map((faq) => <p>{faq.answer}</p>); return ${staticExperience};`,
    ],
    [
    ...
    [
      "literal JSX mentions",
      '<main title="content.hero.image content.services content.faqs">content.hero.image content.services content.faqs{"content.hero.image content.services content.faqs"}{/* content.hero.image content.services content.faqs */}</main>',
    ],
    [
      "discarded expressions",
      `content.hero.image; content.services; content.faqs; console.log(content.hero.image, content.services, content.faqs); return ${staticExperience};`,
    ],
    [
      "discarded comma operands",
      `return (content.hero.image, content.services, content.faqs, ${staticExperience});`,
    ],
    [
      "unused helper",
      `function unused() { return ${validExperience}; } export function Experience() { return ${staticExperience}; }`,
    ],
    [
      "unused arrow",
      `const unused = () => ${validExperience}; return ${staticExperience};`,
    ],
    [
      "local content shadow",
      `const content = { hero: { image: "local" }, services: [], faqs: [] }; return ${validExperience};`,
    ],
    [
    ...
    [
      "callback parameter shadow",
      `export function Experience({ content }) { return unrelated.map((content) => ${validExperience}); }`,
    ],
    [
      "destructured callback shadow",
      `const { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } = content; export function Experience() { return unrelated.map(({ heroImage, serviceItems, faqItems }) => ${aliasedExperience}); }`,
    ],
    [
      "unused callback bindings",
      `export function Experience({ content }) { return unrelated.map(() => { const image = content.hero.image, services = content.services, faqs = content.faqs; return ${staticExperience}; }); }`,
    ],
    [
      "catch parameter shadow",
      `export function Experience({ content }) { try { work(); } catch (content) { return ${validExperience}; } }`,
    ],
    [
      "hoisted function binding shadow",
      `const { hero: { image: heroImage }, services: serviceItems, faqs: faqItems } = content; export function Experience() { if (ready) { var heroImage = "local", serviceItems = [], faqItems = []; } return ${aliasedExperience}; }`,
    ],
    [
      "overwritten aliases",
      `let heroImage = content.hero.image, serviceItems = content.services, faqItems = content.faqs; heroImage = "local"; serviceItems = []; faqItems = []; return ${aliasedExperience};`,
    ],
    [
      "unrelated property names",
      'const unrelated = { "content.hero.image": 1, "content.services": 2, "content.faqs": 3 }; return unrelated;',
    ],
    [
      "similar token prefixes",
      validExperience
  ...
  it("rejects marker values that only contain the expected slug", () => {
    ...
    const report = validateReferenceCandidate({
      referenceDna: dna,
      experienceSource: mismatched,
      stylesSource: validStyles,
      motionSource: validMotion,
    });
    expect(report.pass).toBe(true);
    expect(report.visualPass).toBe(false);
    expect(report.visualFindings).toEqual(
      expect.arrayContaining([
