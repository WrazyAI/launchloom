...
function sourceHasSignature(source, element) {
  ...
  const match = selector.match(
    /data-reference-signature\s*=\s*["']?([a-z0-9_-]+)/iu,
  );
  const value = match?.[1] || element.id;
  return Boolean(
    value &&
    new RegExp(`data-reference-signature\\s*=\\s*["']?${value}\\b`, "iu").test(
      source,
    ),
  );
}
...
function staticStringExpression(source, expression, seen = new Set()) {
  ...
  if (parts.length) {
    const objectBody =
      source.match(
        new RegExp(
          `(?:const|let|var)\\s+${root}\\s*=\\s*\\{([\\s\\S]*?)\\}\\s*;?`,
          "u",
        ),
      )?.[1] || "";
    ...
    return parts.length === 1
      ? staticStringExpression(source, objectProperty, seen)
      : "";
  }
  const declaration =
    source.match(
      new RegExp(`(?:const|let|var)\\s+${root}\\s*=\\s*([^;\\n]+)`, "u"),
    )?.[1] || "";
...
function attributeValue(source, attribute) {
  ...
  const expression = source.match(
    new RegExp(`${attribute}\\s*=\\s*\\{\\s*([^{}]+?)\\s*\\}`, "iu"),
  )?.[1];
...
function outputContentPaths(source) {
  ...
  if (diagnostic) {
    ...
    throw new Error(
      `Reference compliance could not inspect candidate JSX: ${message}`,
    );
  }
    ...
    if (ts.isIdentifier(name)) {
      ...
      name.elements.forEach((element, index) => {
        ...
        bind(
          element.name,
          scope,
          element.dotDotDotToken || key === null ? {} : value,
          [...path, key],
        );
      });
    }
  ...
  for (const statement of file.statements)
    if (
      ...
      entryNames.add(statement.expression.text);
  const isEntryFunction = (node) => {
    if (node.parent === file)
      return exported(node) || entryNames.has(node.name?.text);
    ...
    return (
      ...
      (entryNames.has(declaration.name.getText(file)) ||
        exported(declaration.parent.parent))
    );
  };
  ...
  const index = (node, scope) => {
    ...
    if (
      ...
      if (ts.isFunctionDeclaration(node))
        componentDefinitions.set(node.name.text, node);
    }
  ...
  const append = (paths, keys) =>
    paths.map((path) =>
      [path, ...keys]
        .join(".")
        .replace(/^\$props\.content(?=\.|$)/u, "content"),
    );
  const active = new Set();
  const overrideStack = [];
  const activeComponents = new Set();
  const currentOverrides = () =>
    overrideStack.length ? overrideStack[overrideStack.length - 1] : new Map();
  const componentName = (node) => {
    const tag = node?.tagName;
    return tag && ts.isIdentifier(tag) && /^[A-Z]/u.test(tag.text)
      ? tag.text
      : "";
  };
  const componentBindings = (definition, opening, parentOverrides) => {
    ...
    for (const attribute of opening.attributes.properties) {
      ...
      const expression = ts.isJsxExpression(initializer)
        ...
        : initializer;
      if (expression)
        props.set(attribute.name.text, paths(expression, parentOverrides));
    }
  ...
  const paths = (node, overrides = currentOverrides()) => {
    ...
    if (ts.isIdentifier(node)) {
      ...
      active.add(binding);
      const result = append(
        binding.seed ? [binding.seed] : paths(binding.expression),
        binding.path,
      );
    ...
    if (ts.isPropertyAccessExpression(node))
      return append(paths(node.expression), [node.name.text]);
    if (ts.isElementAccessExpression(node)) {
      const key = node.argumentExpression;
      return key && (ts.isStringLiteral(key) || ts.isNumericLiteral(key))
        ? append(paths(node.expression), [key.text])
        : paths(node.expression);
    }
    if (
    ...
    if (ts.isJsxElement(node))
      return [
        ...paths(node.openingElement, overrides),
        ...node.children.flatMap((child) => paths(child, overrides)),
        ...componentPaths(node, overrides),
      ];
    if (ts.isJsxFragment(node)) return node.children.flatMap(paths);
    if (ts.isJsxSelfClosingElement(node))
      return [
        ...node.attributes.properties.flatMap((property) =>
          paths(property, overrides),
        ),
        ...componentPaths(node, overrides),
      ];
    ...
    if (
      ...
      return paths(node.expression, overrides);
    if (ts.isCallExpression(node)) {
      ...
      return [
        ...
        ...node.arguments.flatMap((argument) =>
          mapped && ts.isFunctionLike(argument)
            ? returnedExpressions(argument.body).flatMap(paths)
            : paths(argument),
        ),
      ];
    }
    if (ts.isConditionalExpression(node))
      return [
        ...paths(node.condition),
        ...paths(node.whenTrue),
        ...paths(node.whenFalse),
      ];
    ...
    if (ts.isTemplateExpression(node))
      return node.templateSpans.flatMap((span) => paths(span.expression));
...
function jsxAttributeText(attribute, source, sourceFile) {
  ...
  return staticStringExpression(source, expression.getText(sourceFile));
}
...
function sourceSectionOrder(source) {
  ...
  const visit = (node) => {
    ...
    if (
      ...
      node.tagName.text.toLowerCase() === "section"
    ) {
      const attributes = node.attributes.properties;
      const marker = attributes.find(
        (attribute) =>
          ts.isJsxAttribute(attribute) &&
          attribute.name.getText(file) === "data-reference-section",
      );
      ...
      const value =
        jsxAttributeText(marker, source, file) ||
        jsxAttributeText(id, source, file);
      if (value) sections.push(slug(value));
    }
    ts.forEachChild(node, visit);
  };
  visit(file);
...
function hasProhibitedPattern(source, pattern) {
  ...
  return new RegExp(
    `\\s(?:data-reference-pattern|data-layout|data-grammar|className|class)\\s*=\\s*["'][^"']*\\b${escaped}\\b[^"']*["']`,
    "iu",
  ).test(source);
}
...
export function validateReferenceContractCompliance({
    ...
    if (!sourceHasSignature(experienceSource, element))
      findings.push(
        finding(
          "missing-signature",
          "critical",
          `Missing required signature ${element.id}.`,
        ),
      );
    if (renderedDom && !sourceHasSignature(renderedDom, element))
      findings.push(
        finding(
          "missing-rendered-signature",
          "critical",
          `Rendered DOM is missing signature ${element.id}.`,
        ),
      );
  }
  ...
  if (
    !/data-hero(?:\s|=)/iu.test(experienceSource) ||
    !/data-hero-geometry=/iu.test(experienceSource)
  )
    findings.push(
      finding(
        "hero-geometry",
        "critical",
        "The authored hero is missing its explicit reference geometry marker.",
      ),
    );
  else if (
    !markerMatches(
      experienceSource,
      "data-hero-geometry",
      referenceDna.heroGeometry.mode,
    )
  )
    findings.push(
      finding(
        "hero-geometry-mismatch",
        "critical",
        "The authored hero geometry does not match Reference DNA.",
      ),
    );
  if (!/data-navigation-geometry=/iu.test(experienceSource))
    findings.push(
      finding(
        "navigation-geometry",
        "major",
        "The authored navigation is missing its reference geometry marker.",
      ),
    );
  else if (
    !markerMatches(
    ...
    findings.push(
      finding(
        "navigation-geometry-mismatch",
        "critical",
        "The authored navigation geometry does not match Reference DNA.",
      ),
    );
  if (!/data-service-presentation=/iu.test(experienceSource))
    findings.push(
      finding(
        "service-presentation",
        "critical",
        "The authored service presentation is missing its reference marker.",
      ),
    );
  else if (
    !markerMatches(
    ...
    findings.push(
      finding(
        "service-presentation-mismatch",
        "critical",
        "The authored service presentation does not match Reference DNA.",
      ),
    );
  if (
    !/data-early-conversion(?:=|\s|>)/iu.test(experienceSource) ||
    !/data-cta-placement=/iu.test(experienceSource)
  )
    findings.push(
      finding(
        "cta-placement",
        "critical",
        "The early CTA is missing its explicit reference placement marker.",
      ),
    );
  else if (
    !markerMatches(
    ...
    findings.push(
      finding(
        "cta-placement-mismatch",
        "critical",
        "The early CTA placement does not match Reference DNA.",
      ),
    );
  if (
    !/data-mobile-recomposition=/iu.test(experienceSource) ||
    !/@media/iu.test(stylesSource)
  )
    findings.push(
      finding(
        "mobile-recomposition",
        "critical",
        "The candidate does not declare a mobile recomposition and responsive CSS.",
      ),
    );
  else if (
    !markerMatches(
    ...
    findings.push(
      finding(
        "mobile-recomposition-mismatch",
        "critical",
        "The mobile recomposition does not match Reference DNA.",
      ),
    );
  if (
    !/mountExperienceMotion\s*\(/u.test(motionSource) ||
    !/data-motion-primitive=/iu.test(experienceSource)
  )
    findings.push(
      finding(
        "motion-primitive",
        "critical",
        "The assigned motion primitive is not represented in the authored candidate.",
      ),
    );
  else if (
    !markerMatches(
    ...
    findings.push(
      finding(
        "motion-primitive-mismatch",
        "critical",
        "The motion primitive does not match Reference DNA.",
      ),
    );
    ...
    if (hasProhibitedPattern(`${experienceSource}\n${renderedDom}`, pattern))
      findings.push(
        finding(
          "prohibited-pattern",
          "critical",
          `Prohibited pattern detected: ${pattern}.`,
        ),
      );
    ...
    if (!match[1].startsWith("ll-creative-"))
      findings.push(
        finding(
          "css-token-collision",
          "critical",
          `Candidate CSS variable --${match[1]} is not isolated.`,
        ),
      );
  ...
  for (const token of [
    ...
    if (
      ...
      findings.push(
        finding(
          "unbound-content-token",
          "critical",
          `Required sealed token ${token} does not flow into output.`,
        ),
      );
  ...
  const score = Math.max(
    ...
    Math.round(
      100 -
        critical * 18 -
        findings.filter((item) => item.severity === "major").length * 8,
    ),
  );
  return {
    version: 1,
...
export function assertReferenceContractCompliance(input) {
  ...
  if (!report.pass)
    throw new Error(
      `Reference fidelity failed: ${report.findings.map((item) => item.message).join(" | ")}`,
    );
  return report;
}
