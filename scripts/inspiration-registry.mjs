...
export function buildInspirationPack(
  ...
  if (requireDossiers) {
    ...
    if (businessMatchedRecords.length < 3)
      throw new Error(
        `The production library has ${businessMatchedRecords.length} eligible dossier(s) matched to '${industry}' out of ${productionRecords.length} eligible references; three structurally independent business-matched dossiers are required. Unrelated industries are not used as filler.`,
      );
