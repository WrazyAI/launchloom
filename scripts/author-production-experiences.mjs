...
BUSINESS CONVERSION INVARIANT
The verified content includes one required primary action: use content.hero.primaryLabel as a functional link to #contact and retain data-early-conversion. Reference DNA describes the source reference, not permission to omit the client's conversion path. If ctaPlacement.early says that no conventional hero CTA is present, keep the action visually quiet and place it at ctaPlacement.primary instead of beside the opening paragraph. Keep the action in the opening data-hero wrapper only when that wrapper's reference-defined sequence reaches its placement; do not force it into the headline cluster or invent a floating CTA. Keep data-cta-placement equal to the canonical marker above.`;
}
...
EXPERIENCE STAGE
Return complete Experience.jsx in content. Export default function Experience({ content, runtime }). Import { LeadForm } from @launchloom/runtime and render exactly one <LeadForm content={content} runtime={runtime} /> inside the section with id="contact". The required primary action is a compact anchor or button using content.hero.primaryLabel, href="#contact", data-early-conversion, and the exact reference placement marker. Never put LeadForm inside the hero, nav, or promise band. Every helper component that reads sealed content must receive content (or a sealed destructured subset) as a prop; never reference a free content variable. Use content tokens for every business fact and every visitor-facing marketing sentence or section heading. Do not place authored marketing words directly between JSX tags. Generic interface labels may be Services, FAQs, Contact, Menu, Open menu, and Close menu. The deterministic host imports and mounts ./motion.js after the component renders; do not import or invoke ./motion.js from Experience.jsx. The deterministic runtime owns root instrumentation. Mark the complete opening composition with data-hero. That region must contain exactly one H1 and the primary data-early-conversion action, so browser verification can measure the actual opening rather than only the navigation. The primary action does not have to sit beside the hero paragraph: when Reference DNA places it in a later first-screen feature/work rail, keep the CTA there and let data-hero wrap the opening sequence through that rail. Keep services, FAQs, and contact outside that measured region. Do not put data-hero on the nav alone. Include data-early-conversion on exactly the primary action, and sections with ids services, faqs, and contact. Use real anchor links href="#services", href="#faqs", and href="#contact" in the navigation; JavaScript-only section buttons are not sufficient. Service detail links must resolve to the real /services/ route using the sealed service slug and a trailing slash. Never turn a service slug into a homepage fragment, because service slugs are real SEO routes, not section IDs. Before returning, confirm the source binds the hero heading, services, and FAQs from content.hero.heading, content.services, and content.faqs, either directly or through destructuring. Use content.hero.image, content.hero.secondaryImage, and content.hero.tertiaryImage for supplied imagery, with descriptive non-claiming alt text. Do not return CSS.
...
STYLES STAGE
Return complete styles.css in content. Return CSS text only, never an HTML document, JSX, markdown fences, or script tags. Style the exact markup without changing its structure. The data-hero region, including the primary heading and early action, must have a measured bounding bottom no greater than the viewport height at 1536x864 and 1366x768 at 100 percent zoom. Use a compact hero composition: one headline, short body, one early CTA, and the assigned family's image treatment or atmospheric field. Do not make the hero grow to accommodate a contact form, service list, or long copy. Avoid large fixed padding and min-heights that exceed the viewport; use min-height: 0 where content can wrap. Recompose for 390x844 without horizontal overflow. Include visible focus, adequate contrast, readable body type, and prefers-reduced-motion. Use no remote URLs.`;
