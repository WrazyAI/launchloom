...
export function assertReferenceDossierPack(pack, { repositoryRoot = process.cwd() } = {}) {
  ...
  for (const route of pack.routes) {
    ...
    if (
      ...
      stableJson(actual.tags) !== stableJson(bound.tags) ||
      stableJson(actual.source) !== stableJson(bound.source)
    )
    ...
    if (
      stableJson(curatedDnaContract(dna)) !==
      stableJson(curatedDnaContract(actual.referenceDna))
    )
      throw new Error(`Production route '${route.id}' curated Reference DNA differs from its dossier source.`);
  }
  return pack;
}
