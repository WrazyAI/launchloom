...
export default function ReviewPanel() {
  ...
  async function submitFeedback(event: FormEvent) {
    ...
    const response = await fetch("/api/feedback", {
      ...
      body: JSON.stringify({ token, comment, pageUrl: document.referrer || window.location.href }),
    });
  ...
  return <main className="review-shell"><div className="review-card"><span className="eyebrow">LaunchLoom review</span><h1>Make this site yours.</h1><p>Leave feedback in plain language. We’ll group your notes into one clean revision.</p><form onSubmit={submitFeedback}><label className="field">What should change?<textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="For example: Use ‘same-day garage door repair’ in the headline and make the phone number more prominent." /></label><button className="button" type="submit">Send feedback</button></form><div className="review-divider" /><h2>Happy with this version?</h2><p>Approval publishes this exact preview. A newer revision will require a new approval link.</p><button className="button secondary" type="button" onClick={approve}>Approve & publish</button>{state && <p className="form-message success">{state}</p>}</div></main>;
}
